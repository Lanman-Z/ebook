...
app = FastAPI(title="EBook Demo App")
...
async def upload_book(
    ...
    if suffix not in ALLOWED_SUFFIXES:
        raise HTTPException(status_code=400, detail="Please upload a PDF or EPUB file.")
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to parse book: {exc}") from exc

    if not content.strip():
        save_path.unlink(missing_ok=True)
        raise HTTPException(status_code=400, detail="The uploaded book does not contain readable text.")
...
async def create_review(
    ...
    if not selected:
        raise HTTPException(status_code=400, detail="Please select a sentence or passage first.")
    ...
    with get_connection() as conn:
        ...
        cursor = conn.execute(
            """
            INSERT INTO reviews (book_id, reviewer, selected_text, comment, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (book_id, reviewer_name, selected, review_comment, created_at),
        )
